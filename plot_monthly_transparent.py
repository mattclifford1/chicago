...
def main():
    ...
    for i in range(len(months)):
        ...
        plt.yticks([])
        plt.title(monthname[i])
        plt.savefig('heatmaps_transparent_' + monthname[i] + '.png',bbox_inches='tight')
...
def getData(primarytype):
    dataCoord = np.load('dataCoordmonth' + primarytype + '.npy')    #load coordinate data
    X = dataCoord[0,:]                      #and separate
    Y = dataCoord[1,:]
    IUCR = np.load('IUCRmonth' + primarytype + '.npy')        #load severity
