...
def main():
    i = ['ARSON','ASSAULT','BATTERY','CRIM SEXUAL ASSAULT','HOMICIDE','KIDNAPPING','OFFENSE INVOLVING CHILDREN','NARCOTICS','SEX OFFENSE','ROBBERY']
    for j in i:
...
def makeHeatmap(X, Y, sev, data_reduce):
    ...
    for i in range(len(X)):
        heatIm[abs(int(y_shift[i])-heatIm.shape[0]+1), int(x_shift[i])] += 1
            
        #heatIm = heatIm/np.max(heatIm)
        #plot and save image
    plt.figure(figsize=(6,6))
    img = sc.misc.imread('chicago_image.jpg')
    plt.imshow(img, zorder=0, extent=[0, 112, 139, 0])
