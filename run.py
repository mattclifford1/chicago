...
def main():
	...
	if mixture == True:
		...
		n_components = [5,10,14,18,23,30,40,50, 70,90,100,120,140,155,170,200,300,400]
		n_components = [300,400,500]
		...
		np.save('lastModels.npy',models)
		
		# plt.clf()
		m = np.load('modelsFullNew.npy')
		models = np.append(m, models)
		n_components = [5,10,14,18,23,30,40,50, 70,90,100,120,140,155,170,200,300,400]
		a=[m.bic(sevData) for m in models]
		b=[m.aic(sevData) for m in models]
...
def getData():
	dataCoord = np.load('dataCoordmonth08.npy')    #load coordinate data
	X = dataCoord[0,:]                      #and separate
	Y = dataCoord[1,:]
	IUCR = np.load('IUCRmonth08.npy')        #load severity
